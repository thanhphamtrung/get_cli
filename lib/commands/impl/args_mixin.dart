...
mixin ArgsMixin {
  final List<String> _args = DexCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(DexCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return DexCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < DexCli.arguments.length) {
      index++;
      return DexCli.arguments[index];
    } else {
